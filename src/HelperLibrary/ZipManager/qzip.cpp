...
Q_DECLARE_TYPEINFO(QZipAvia::CentralFileHeader, Q_PRIMITIVE_TYPE);
...
LocalFileHeader QZipAvia::CentralFileHeader::toLocalHeader() const
{
    LocalFileHeader h;
...
void ZipReaderPrivate::scanFiles()
{
    ...
    for (i = 0; i < num_dir_entries; ++i) {
        FileHeader header;
        int read = device->read((char *) &header.h, sizeof(QZipAvia::CentralFileHeader));
        if (read < (int)sizeof(QZipAvia::CentralFileHeader)) {
...
void ZipWriterPrivate::addEntry(EntryType type, const QString &fileName, const QByteArray &contents/*, QFile::Permissions permissions, QZip::Method m*/)
{
    ...
    FileHeader header;
    memset(&header.h, 0, sizeof(QZipAvia::CentralFileHeader));
...
void ZipWriter::insertDirectory(const QString &_dirName, bool _log)
{
    QDir dir(_dirName);

    if(_log)
    {
        qInfo() << QString("Start compress data from [%1]").arg(_dirName);
    }

    QDirIterator it(_dirName, QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot, QDirIterator::Subdirectories);
    while (it.hasNext())
    {
        it.next();

        const QString relFileName = dir.relativeFilePath(it.filePath());
        ...
        if(fInfo.isFile())
        {
            ...
            {
                qWarning() << QString("Error while open file [%1] - [%2]")
            ...
            if(_log)
            {
                qInfo() << QString("Add file [%1]").arg(relFileName);
            }

            addFile(relFileName, &sourceFile);
        }

        if(fInfo.isDir())
        {
            addDirectory(relFileName);

            if(_log)
            {
                qInfo() << QString("Add directory [%1]").arg(relFileName);
            }
        }
    }
}
...
void ZipWriter::close()
{
    ...
    for (int i = 0; i < d->fileHeaders.size(); ++i) {
        ...
        d->device->write((const char *)&header.h, sizeof(QZipAvia::CentralFileHeader));
